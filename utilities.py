...
def save(con,data,filename=None):
	...
	if hasattr(con,"_pricewalk"):
		kwargs['price'] = con._pricewalk
	if hasattr(con._agents[0],'util_gain'):
		kwargs['util'] = np.array([a.util_gain for a in con._agents])
		kwargs['cost'] = np.array([a.foo for a in con._agents])
